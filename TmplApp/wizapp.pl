...
our $TMPL_APP_NAME = 'TmplApp';
our $TMPL_APP_NAME_S = 'tmpl_app';
...
sub add_controller {
    ...
    $fcnan =~ s/_::/::/g;
    $fcnan =~ s/::/\//g;
    my $ppcan = filename $pcan;
    my $pfcnan = filename $fcnan;
    my $model_r = qq|s#our \\\$MODEL = 'AllForm';#our \\\$MODEL = '$ppcan';#|;
    my $autoform_r = qq|s#our \\\@AUTOFORM = qw\\(all_form#our \\\@AUTOFORM = qw\\($pfcnan#|;
    ...
    if ($args->{a}) {
        ...
        if (!-d "$dest$an/autoform/default/$fcnan.pds") {
            my @replace = @{$conf{replace}};
            push @replace, "s/all_form/$fcnan/g";
            replace_copy(
                "$td/autoform/default/all_form.pds",
                "$dest$an/autoform/default/$fcnan.pds",
                \@replace, $conf{ignore},
            );
        }
    }
