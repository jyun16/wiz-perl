...
(function($) {
	...
	$.jCal = function (target, opt) {
		opt = $.extend({
            ...
            callbackSelectYearOrMonth: function (year, month) { return true; },
	...
	function drawCalControl (target, opt) {
			...
			$(target).find('.jCal .monthName, .jCal .monthYear')
				...
				.bind('click', $.extend( {}, opt ),
					function (e) {
						...
						for (var di = ( monthName ? 0 : opt.startYearOffset ), dd = ( monthName ? 12 : opt.endYearOffset ); di < dd; di++)
						...
						$('.jCalMo .monthSelect', e.data._target).bind('mouseover mouseout click', $.extend( {}, e.data ), 
							function (e) {
								if ( e.type == 'click' ) {
                                    var re = $(this).attr('id').match("(.*)_(.*)_(.*)");
                                    opt.callbackSelectYearOrMonth(re[3], re[1]);
		...
		$(target).find('.jCal .left').bind('click', $.extend( {}, opt),
			function (e) {
                ...
                opt.callbackSelectYearOrMonth(e.data.day.getFullYear(), e.data.day.getMonth() + 1);
